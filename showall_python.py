...
#        NULL`"", 'scopes-resources',	# : "/rest/index/resource,s?query=`"NOT scopeUris
    ...
    app = "10.10.10.1"      # better as a global
    datadir = "sysconfig"   # probably better as a global
    datafile = "appliance"
...
def postCall(class_s, post):
    resp = None
    body = class_s.body
    uri = "https://{}/{}/".format(class_s.app, class_s.SessionDict[post])
    print("POST: ", uri)
    try:
        resp = requests.post(uri, headers=class_s.post_headers, data=json.dumps(body), verify=False)
        if (resp.status_code != 200):
            print("OV Connection ..... Failed: ", resp.status_code)
    except Exception as e:
        print(f"Unable to reach OneView appliance. Reason  - {e}.")
